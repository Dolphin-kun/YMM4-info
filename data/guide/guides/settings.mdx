...
- `data/guide/xxx/xxx.mdx`と`pages/guide/[...slug].tsx`
...
`globalRoutes`と`guideRoutes`が格納されています。それぞれ

- `globalRoutes`：サイト全体の案内
- `guideRoutes`：ドキュメント内のリンク
...
コンポーネントの登録の仕方は
[Using MDX with Next.js](https://nextjs.org/guide/advanced-features/using-mdx)
や
